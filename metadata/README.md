...
- `local/contracts/field1_investigator_vegetation_reference_contract.yaml`:
  freezes the accepted investigator point layer, point-level QC dispositions,
  cross-cube transfer results, spectral profile, and visual QC. Any held-out
  cube below the per-cube warning thresholds remains named in this contract;
  downstream use is limited to polygon-constrained probability review until a
  separate mask decision is accepted.
### Investigator chickpea probability review

After the investigator vegetation-reference contract is frozen,
`scripts/build_investigator_chickpea_probability_maps.py` writes continuous,
review-only `chickpea_review_probability.tif` layers under
`data/processed/investigator_chickpea_probability_review/`. Summary tables and
PNG QC are stored under
`metadata/local/reports/mask_refinement/investigator_chickpea_probability_review/`,
