(function (root) {
  ...
  const Model = {addPolygon, deletePolygon, moveVertex, insertVertex, deleteVertex, movePolygon, history, historyApply, historyUndo, historyRedo, selfIntersects, polygonsOverlap, pointInPolygon};
  ...
  let drawing = [], hoverPoint = null, pointerAction = null, opacity = .25;
  ...
  function fullPosition(event) {
    ...
    return {x: view.center.x + (position.x - canvas.width / 2) / map.zoom, y: view.center.y + (position.y - canvas.height / 2) / map.zoom};
  }
  ...
  function render() {
    ...
    if (drawing.length) {
      ...
      context.save(); context.strokeStyle = COLORS[selectedZone]; context.lineWidth = 2; context.beginPath();
      ...
      for (const point of drawing) { context.beginPath(); context.arc(map.x(point.x), map.y(point.y), 4, 0, Math.PI * 2); context.fillStyle = COLORS[selectedZone]; context.fill(); }
      context.restore();
    }
    renderAudit();
  }
  function renderAudit() {
    ...
    $("audit").textContent = `${polygons.length} polygons · ${selfCount} self-intersections · ${overlaps} incompatible overlaps · ${outsideCount} beyond-raster warnings${outsideCount ? " (exact clipped fraction is written to the saved audit)" : ""}`;
    $("audit").className = selfCount || overlaps || outsideCount ? "help warning" : "help ok";
  }
  ...
  function finishPolygon() {
    if (drawing.length < 3) return setStatus("A polygon requires at least three vertices.", true);
    const polygon = {polygon_id: `${cubeId()}-polygon-${Date.now()}`, zone_type: selectedZone, vertices_pixel: clone(drawing)};
    mutatePolygons(addPolygon(record().polygons, polygon)); selectedPolygon = polygon.polygon_id; drawing = []; hoverPoint = null; tool = "edit"; updateTool();
  }
  function updateTool() {
  ...
  function setTool(name) { tool = name; drawing = name === "draw" ? drawing : []; hoverPoint = null; updateTool(); render(); }
  ...
  function canNavigate() { if (!drawing.length) return true; setStatus("Finish or discard the polygon before changing cubes.", true); return false; }
  function navigate(delta) {
    if (!canNavigate()) return; cubeIndex = Math.max(0, Math.min(manifest.length - 1, cubeIndex + delta));
    $("cube").value = cubeId(); selectedPolygon = ""; drawing = []; loadImage(true);
  }
  async function save() {
    if (drawing.length) return setStatus("Finish or discard the active polygon before saving.", true);
  ...
  canvas.addEventListener("pointerup", (event) => {
    ...
    if (pointerAction.kind === "draw") { drawing.push(fullPosition(event)); markDirty(); }
  ...
  $("draw").onclick = () => setTool("draw"); $("finish").onclick = finishPolygon; $("edit").onclick = () => setTool("edit");
  ...
  $("undo").onclick = () => { const value = historyUndo(historyForCube()); if (value !== historyForCube()) {syncHistory(value); markDirty(); render();} };
  $("redo").onclick = () => { const value = historyRedo(historyForCube()); if (value !== historyForCube()) {syncHistory(value); markDirty(); render();} };
  ...
  document.addEventListener("keydown", (event) => {
    if (event.key === "Enter" && tool === "draw") {event.preventDefault(); finishPolygon();}
    if (event.key === "Escape") {drawing = []; hoverPoint = null; pointerAction = null; render();}
  ...
  async function init() {
    ...
    for (const zone of schema.zone_types) {
      const button = document.createElement("button"); button.type = "button"; button.dataset.zone = zone; button.className = `zone-${zone}`; button.textContent = zone.replaceAll("_", " "); button.onclick = () => {selectedZone = zone; setTool("draw"); renderForm();}; $("zoneButtons").appendChild(button);
  ...
  root.__field2AreaDebug = {getRecord: () => clone(record()), getView: () => clone(view), getTool: () => tool};
