...
let manifest=[],state={version:1,cubes:{}},referenceState={cubes:{}},image=new Image(),drawing=[],pointer=null,dirty=false,activeCube='',activeLayer=layer.value;
...
function loadImage(){drawing=[];pointer=null;activeCube=cube.value;activeLayer=layer.value;image=new Image();image.onload=()=>{canvas.width=image.naturalWidth;canvas.height=image.naturalHeight;draw();const item=manifest.find(x=>x.cube_id===activeCube);setStatus(activeCube+' • '+item.analysis_role+' • '+polygons().length+' polygons'+(dirty?' • unsaved':''))};image.onerror=()=>setStatus('Layer could not be loaded.',true);image.src=imageUrl()+'?v='+Date.now()}
...
function finish(){if(drawing.length<3){setStatus('A polygon needs at least three points.',true);return false}polygons().push({id:activeCube+'-'+Date.now(),kind:kind.value,layer:activeLayer,note:note.value.trim(),points:drawing.map(p=>({x:+p.x.toFixed(3),y:+p.y.toFixed(3)}))});drawing=[];pointer=null;note.value='';markDirty();draw();return true}
...
cube.onchange=()=>{if(!canNavigate()){cube.value=activeCube;return}loadImage()};layer.onchange=()=>{if(!canNavigate()){layer.value=activeLayer;return}loadImage()};kind.onchange=draw;references.onchange=draw;
